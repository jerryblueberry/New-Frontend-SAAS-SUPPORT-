...
const DashboardAvailability = React.memo(function DashboardAvailability({ onboardingData, onEdit, setModalOpen, modalOpen }) {
  ...
  // Reusable compact stat card for cleaner UI and consistency
  const StatCard = useCallback(({ color, Icon, value, label, ariaLabel }) => (
    <Paper
      role="group"
    ...
    >
      <Stack direction="row" spacing={{ xs: 1, sm: 1.25 }} alignItems="center" sx={{ width: '100%', minWidth: 0 }}>
        <Box
          sx={{
            width: { xs: 22, sm: 26 },
            height: { xs: 22, sm: 26 },
        ...
        >
          <Icon sx={{ fontSize: { xs: 14, sm: 16 }, color: `${color}.main` }} />
        </Box>
          ...
          >
            ...
            <Grid item xs={12} sm={6} md={3}>
              <StatCard
                color="info"
                Icon={TrendingUpIcon}
                value={`${Math.round((availableDays.length / DAYS_OF_WEEK.length) * 100)}%`}
                label="Coverage"
